...
public class StaffManagementViewModel : ViewModelBase
{
    ...
    public ObservableCollection<string> SortableProperties
    {
        get;
    }
    ...
    public ICommand RestoreStaffCommand
    {
        get;
    }
    public ICommand AssignTaskCommand
    {
        get;
    }
    ...
    public StaffManagementViewModel()
    {
        ...
        SortableProperties = new ObservableCollection<string> {
            "FullName",
            "Username",
            "Email",
            "Role"
    ...
    private async Task OpenAssignTaskDialog(User? staffToAssign)
    {
        ...
        // Load danh sách user có thể gán (nếu cần, mặc dù ở đây đã có sẵn user)
        await addEditTaskViewModel.LoadAssignableUsersAsync();

        addEditTaskViewModel.InitializeForAdd(); // Khởi tạo dialog để thêm task mới
        addEditTaskViewModel.SelectedAssignee = staffToAssign; // Gán trước assignee
        ...
        {
            Title = "Assign New Task to " + staffToAssign.FullName, // Tiêu đề dialog
            Content = dialogView,
            Width = 520,
            Height = 650,
        ...
        addEditTaskViewModel.CloseActionWithResult = (success) =>
        {
            ...
            if (success)
            {
                // Không cần LoadStaffAsync() ở đây, vì chúng ta đang ở StaffManagement
                // Có thể hiển thị toast thành công
                _toastViewModel.Show($"Task assigned to {staffToAssign.FullName} successfully.", ToastType.Success);
            }
        };
    ...
    private async Task OpenAddEditStaffDialog(User? userToEdit)
    {
        ...
        var dialogView = new AddEditUserDialog
        {
            DataContext = addEditUserVM
        };
        var dialogWindow = new Window
        {
    ...
    private async Task DeleteStaffAsync(User? staffToDelete)
    {
        ...
        try
        {
            ...
            {
                _toastViewModel.Show($"Failed to mark staff '{staffToDelete.FullName}' inactive.", ToastType.Error);
            }
        }
        catch (Exception ex)
        {
            _toastViewModel.Show($"Error deleting staff: {ex.Message}", ToastType.Error);
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task RestoreStaffAsync(User? staffToRestore)
    {
        ...
        {
            _toastViewModel.Show($"Error restoring staff: {ex.Message}", ToastType.Error);
        }
        finally
        {
            IsLoading = false;
